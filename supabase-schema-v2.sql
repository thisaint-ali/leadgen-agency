...
create table if not exists email_queue (
  ...
  prospect_id uuid,                    -- reference to prospects (no FK to avoid dependency order issues)
  to_name text,
...
create table if not exists master_conversations (
  id uuid default gen_random_uuid() primary key,
  created_at timestamptz default now(),
  msg_role text not null,              -- user | assistant  (renamed: 'role' is reserved in PostgreSQL)
